...
class MyRoleViewController: UIViewController {
	...
	private func prepareFormData(shouldAddEmptyPlayCell: Bool = false,shouldAddEmptyCoachCell: Bool = false) -> [FormCellType] {
		...
		if let cell = tableView.dequeueReusableCell(withIdentifier: AddAnotherSportTableViewCell.reuseId, for: indexPath) as? AddAnotherSportTableViewCell {
			if (tableView.cellForRow(at: indexPath) as? AddAnotherSportTableViewCell) != nil {
				if cell.titleLabel.text == "+ Add Another sport you play" {
					...
					self.formData = prepareFormData(shouldAddEmptyCoachCell: true)
					tableView.reloadData()
				}
